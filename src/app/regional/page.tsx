...
import { Box, Typography, Card, CardContent, Grid, Button, Divider, List, ListItem, ListItemText, Chip } from '@mui/material';
import { TrendingUp, School, People, Settings, Announcement, Church, Refresh } from '@mui/icons-material';
...
export default function RegionalDashboard() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUser(u);
      if (u.role !== 'REGIONAL_ADMIN') router.push('/login');
      else loadDashboard();
    } else {
      router.push('/login');
    }
  ...
  const loadDashboard = async () => {
    setLoading(true);
    try {
      const res = await api.get('/regional/dashboard');
      setStats({
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      <Box sx={{ width: 220, bgcolor: '#fff', borderRight: 1, borderColor: 'divider', p: 2 }}>
        ...
        <Typography variant="caption" color="text.secondary">Regional Dashboard</Typography>
        <Divider sx={{ my: 2 }} />
        <List dense>
          <ListItem disablePadding><ListItemText primary="Dashboard" /></ListItem>
          <ListItem disablePadding><ListItemText primary="Districts" /></ListItem>
          <ListItem disablePadding><ListItemText primary="Announcements" /></ListItem>
      ...
      <Box sx={{ flex: 1, p: 3, bgcolor: '#f5f5f5' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
          <Box>
            ...
            <Typography color="text.secondary">Regional Overview</Typography>
          </Box>
        ...
        <Card>
          <CardContent>
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
              <Button variant="contained">Add District</Button>
